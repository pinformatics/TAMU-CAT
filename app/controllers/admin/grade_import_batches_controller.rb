...
class Admin::GradeImportBatchesController < Admin::BaseController
  ...
  def batch_audit_notifier
    @batch_audit_notifier ||= GradeImports::BatchAuditNotifier.new(batch: @batch, actor: current_user, request_path: request.fullpath)
  end

  def record_grade_import_activity!(import_action, description, metadata = {})
    batch_audit_notifier.record_activity!(import_action, description, metadata)
  end

  def notify_advisors_of_course_data_update!(action_label)
    batch_audit_notifier.notify_advisors_of_course_data_update!(action_label)
  end

  def notify_admins_of_grade_import_review_if_needed!(action_label)
    batch_audit_notifier.notify_admins_of_grade_import_review_if_needed!(action_label)
  end

  def notify_admins_of_grade_import_failure!(error_message)
    batch_audit_notifier.notify_admins_of_grade_import_failure!(error_message)
  end

  def notify_admins_of_missing_grade_import_semester!
    batch_audit_notifier.notify_admins_of_missing_grade_import_semester!
  end
