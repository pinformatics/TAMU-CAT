...
- **Two safety nets exist for when a large grade import gets killed by the dyno anyway.** (1) `GradeImports::BatchProcessor` checks process RSS every `MEMORY_GUARD_CHECK_INTERVAL_ROWS` rows during the Canvas outcomes row-write loop (`process_memory_limit_exceeded?`, default ceiling `DEFAULT_MEMORY_GUARD_LIMIT_MB` = 800MB, overridable via `GRADE_IMPORT_MEMORY_LIMIT_MB`); if crossed, it stops cleanly, keeps every row already committed, and records a `type: "memory_limit"` parse error telling the admin to re-upload the same file (already-imported rows are skipped automatically via the existing import-fingerprint duplicate check) -- this turns an abrupt SIGKILL into a graceful partial import instead of silence. (2) Because a dyno-level kill still bypasses the job's own `rescue`/`ensure` (the whole process dies, not just a Ruby exception), a batch can still get abandoned at `status: "processing"` forever. `GradeImports::StaleBatchWatchdog` (recurring task in `config/recurring.yml`, every 15 minutes) marks any batch still `"processing"` after `STALE_AFTER` (45 minutes) as `"failed"` and notifies admins, so this is discovered automatically instead of only when someone manually checks.
